...
// Built-in defaults.
//
// GENERATED from config/silver.json at build time (see the Makefile rule for
// silver_config_default.h). It used to be a second hand-maintained copy of the
// same JSON, and the two drifted: a duplicate "viewer" key here silently shadowed
// wheelZoomStep, and every new setting had to be added twice or the defaults were
// wrong for anyone running without a config file.
#include "silver_config_default.h"
